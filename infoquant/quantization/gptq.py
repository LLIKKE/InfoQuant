            ...
            for name in subset:
                ...
                if "R1R4" in name:
                    if ".0" in name or ".1" in name or ".2" in name:
                        ...
                        if args.residual_bits >= 16:
                            continue
                        layer_weight_bits = args.residual_bits
                        sequential.append(["R1R4.module"])
                if "R4R6" in name:
                    if ".0" in name or ".1" in name or ".2" in name:
                        layer_weight_bits = 16
                        continue
...
@torch.no_grad()
def gptq_fwrd_layer(layer,layer_idx, inps,attention_mask,position_ids, dev, args):
    """
    From GPTQ repo
    """
    logging.info("-----GPTQ Quantization-----")
...
@torch.no_grad()
def rtn_fwrd_layer(layer, dev, args):
    """
    From GPTQ repo
    """
    # assert args.w_groupsize == -1, "Groupsize not supported in RTN!"
