...
class ActQuantizer(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        s, z = self.find_params(x)
        x_dtype = x.dtype
        if self.sym:
            return STEQuantize.apply(x, self.scale, self.maxq).to(x_dtype)

        return s * (torch.clamp(round_ste(x / s) + z, 0, self.maxq) - z)
    ...
    def configure(
            self, bits: int, groupsize: int = -1, sym: bool = False, clip_ratio: float = 1.0, lac: bool = False,
            group_num: int = -1
    ) -> None:
        ...
        self.lac = lac
        if self.lac:
            init_value = -math.log(1 / (self.clip_ratio) - 1)
            self.sigmoid = torch.nn.Sigmoid()
            if groupsize > 0:
                self.clip_factor_a_max = torch.nn.Parameter(torch.ones((self.group_num, 1)) * init_value,
                                                            requires_grad=True)
                self.clip_factor_a_min = torch.nn.Parameter(torch.ones((self.group_num, 1)) * init_value,
                                                            requires_grad=True)
            else:
    ...
    def find_params_per_token_groupwise(self, x):  # -> None:
        ...
        if self.lac:
            xmax = torch.amax(reshaped_x, dim=3, keepdim=True) * (1 + self.sigmoid(self.clip_factor_a_max)) * 0.5
            xmin = torch.amin(reshaped_x, dim=3, keepdim=True) * (1 + self.sigmoid(self.clip_factor_a_min)) * 0.5
        else:
    ...
    def find_params(self, x):
        ...
        if self.groupsize > 0:
            ...
            s, z = self.find_params_per_token_groupwise(x)
        ...
        if self.lac:
            xmax = xmax * (1 + self.sigmoid(self.clip_factor_a_max)) * 0.5
            xmin = xmin * (1 + self.sigmoid(self.clip_factor_a_min)) * 0.5
        else:
            xmin = xmin * self.clip_ratio
            xmax = xmax * self.clip_ratio
